...
import land.sungbin.composablepaparazzi.SnapshotImageProvider.Companion.dri
import okio.Path
...
class SnapshotAwareKotlinSignatureProvider(context: DokkaContext) : SignatureProvider {
  ...
  init {
    ...
    checkNotNull(configuration) {
      "The 'land.sungbin.composablepaparazzi.ComposablePaparazziPlugin' Dokka configuration is missing."
    }
    ...
    val path = checkNotNull(values[SnapshotImageProvider.CONFIGURATION_PATH_KEY]) {
      "The 'snapshotImageDir' field in the 'land.sungbin.composablepaparazzi.ComposablePaparazziPlugin' Dokka configuration is missing."
    }
